...
void catchError(int param, int code, char* msg)
{
	if(param)
	{
		//printf("Error Code %d: %s\n", code, msg);
                //f_printf( &MyFileLog, "Error Code %d:", code);
                //f_printf( &MyFileLog, " %s", msg);
                //f_printf( &MyFileLog, "\n");
		Error_Status = 1;
	}
}
...
int (*handler_g03)(int*, double, double, double, double, double);
...
extern int parse_start()
{	
	setFlags=malloc(sizeof(int)*12);
        memset (setFlags, 0, sizeof (int)*12);
	Error_Status=0;
	int code=E();
	free(setFlags);
	return code;
